...
Film* lireFilm(istream& fichier, const ListeFilms& listeFilms)
{
	...
	*ptrFilm = film;
	...
	for (auto&& i : range(0, film.acteurs.nElements)) 
	{
		...
		ajouterFilm(ptrFilm, ptrActeur->joueDans);
	}
...
void detruireFilm(Film* filmADetruire)
{
	...
	for (auto i : range(filmActuel.acteurs.nElements - 1, -1, -1)) {
		...
		if (filmActuel.acteurs.elements[i]->joueDans.nElements == 0) {
			if (filmActuel.acteurs.elements[i]->joueDans.elements != nullptr) {
				delete[] filmActuel.acteurs.elements[i]->joueDans.elements;
			}
...
int main()
{
	...
	Film* ptrAlien = listeFilms.elements[0];
	enleverFilm(ptrAlien, listeFilms);
